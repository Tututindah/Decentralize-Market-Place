...
export function useCardano() {
  ...
  const refreshBalance = useCallback(async () => {
    ...
    try {
      const balance = await cardanoService.getBalance();
      setWallet((prev: typeof wallet) => ({ ...prev, balance }));
    } catch (error: any) {
  ...
  const acceptBidAndCreateEscrow = useCallback(
    async (params: {
      ...
      try {
        ...
        const result = await cardanoService.createEscrow({
          ...params,
        ...
        // Wait for confirmation
        toast.loading("Waiting for confirmation...", { id: result.txHash });
        const confirmed = await cardanoService.waitForConfirmation(result.txHash);
        
        if (confirmed) {
          toast.success("Escrow confirmed!", { id: result.txHash });
        } else {
          toast.error("Confirmation timeout", { id: result.txHash });
        }
