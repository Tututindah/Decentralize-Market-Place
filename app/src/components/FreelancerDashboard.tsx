...
export function FreelancerDashboard({ onJobSelect, onShowProfile, onSettingProfile, onGetStarted }: FreelancerDashboardProps) {
  ...
  const cardClass = isDarkMode ? 'bg-zinc-900 border border-primary/30' : 'bg-gray-100 border border-gray-300';
  const jobCardClass = isDarkMode ? 'bg-zinc-900 border border-white/10 hover:bg-zinc-800' : 'bg-white border border-gray-200 hover:bg-gray-50';
          ...
          <div className="lg:col-span-3">
            <Card className={`p-6 ${cardClass} space-y-4 sticky top-20`}>
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="icon" onClick={onSettingProfile} className={isDarkMode ? 'text-white hover:bg-zinc-800' : 'text-black hover:bg-gray-200'}>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <Badge variant="secondary" className={`capitalize ${isDarkMode ? 'text-black border-primary/50' : 'text-black border-primary/50'}`}>
                          {job.status}
                        </Badge>
                      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 mb-6">
                <Input 
                  ...
                  className={`flex-grow ${isDarkMode ? 'bg-zinc-800 border-zinc-800 text-white' : 'bg-gray-100 border-gray-300 text-black'}`} 
                />
                <Button variant="outline" size="icon" className={isDarkMode ? 'border-zinc-700 hover:bg-zinc-800' : 'border-gray-300 hover:bg-gray-200'}>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                          <h3 className={textForegroundClass}>{job.title}</h3>
                          <Badge variant="outline" className={`capitalize ${isDarkMode ? 'border-zinc-700 text-white' : 'border-gray-300 text-black'}`}>
                            {job.status}
                          </Badge>
                        </div>
                        <p className={textMutedClass + ' mb-3'}>{job.description}</p>
