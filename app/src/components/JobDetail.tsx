...
export function JobDetail({ job, userType, onBack, onWorkSubmission, onApproval, onDispute, isDarkMode, onToggleTheme, onGetStarted, onShowProfile }: JobDetailProps) {
  ...
  const inputClass = isDarkMode ? 'bg-zinc-800 border-zinc-800 text-white' : 'bg-gray-100 border-gray-300 text-black';
  ...
  return (
    <div className={`min-h-screen ${rootClass} transition-colors flex flex-col`}>
        ...
        <div className="flex-grow max-w-7xl mx-auto px-4 py-8 w-full">
            <Button variant="ghost" onClick={onBack} className={`mb-6 ${isDarkMode ? 'text-white hover:bg-zinc-800' : 'text-black hover:bg-gray-200'}`}>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <Card className={`p-6 ${cardClass} space-y-4`}>
                        <div className="flex justify-between items-start">
                            ...
                            <Badge 
                                ...
                                className={`capitalize text-lg px-4 py-1 ${job.status === 'dispute' ? 'bg-red-600 text-white hover:bg-red-700' : isDarkMode ? '' : 'text-black border-gray-300'}`}
                            >
                    ...
                    {job.status === 'open' && userType === 'employer' && (
                        <Card className={`p-6 ${cardClass} space-y-4`}>
                            ...
                            {mockBids.map((bid) => (
                                <div key={bid.id} className={`p-4 rounded-lg flex justify-between items-center transition-colors ${isDarkMode ? 'bg-zinc-900 border border-white/10' : 'bg-white border border-gray-200'}`}>
                                    <div>
                    ...
                    {job.status === 'open' && userType === 'freelancer' && showBidForm && (
                        <Card className={`p-6 ${cardClass} space-y-4`}>
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                <Button variant="outline" onClick={() => setShowBidForm(false)} className={isDarkMode ? 'border-zinc-700 hover:bg-zinc-800' : 'border-gray-300 hover:bg-gray-200'}>
                                    Cancel
                                </Button>
